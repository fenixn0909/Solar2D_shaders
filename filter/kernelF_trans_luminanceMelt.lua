...
--[[
  ...
  https://gl-transitions.com/editor/luminance_melt
  License: MIT
...
P_COLOR vec4 FragmentKernel( P_UV vec2 texCoord )
{
  ...
  if (progress == 0.0) {
    ...
    if(above){
     ...
     m = dist <= r && luminance(cFrom)>l_threshold ? 1.0 : (progress*progress*progress);
    }
    else{
     //m = dist <= r && luminance(getFromColor(p))<l_threshold ? 1.0 : (progress*progress*progress);  
     m = dist <= r && luminance(cFrom)<l_threshold ? 1.0 : (progress*progress*progress);  
