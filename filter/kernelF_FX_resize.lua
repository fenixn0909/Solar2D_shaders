...
// ブラー強度
float v_sizeScale = 0.01; //: hint_range( 0.0, 1.0 ) best 0.0, 0.2
// サンプリング回数
uniform int sampling_count = 12; //: hint_range( 1, 64 ) 
...
P_COLOR vec4 FragmentKernel( P_UV vec2 texCoord )
{
  ...
  //Test 
  v_sizeScale = abs(sin(CoronaTotalTime*2)*1) * 0.45 - 0.4;

  vec2 direction = SCREEN_UV - blur_center;
  vec3 c = vec3( 0.0, 0.0, 0.0 );
  float f = 1.0 / float( sampling_count );
  for( int i=0; i < sampling_count; i++ ) {
    c += texture2D( CoronaSampler0, SCREEN_UV - v_sizeScale * direction * float(i) ).rgb * f;
  }
  COLOR.rgb = c;
