...
--[[
    Status Bar Overlay< With Color Tween + Grids >

    Creat a ImageRect by the size and image file you want, apply and tweak it as you like.
...
float Num_Grids = CoronaVertexUserData.z;

float NB_UsingOverlay = 1.0; // 1: Enable, others: Disable.
float Ovl_Col_Rate = 0.25;
float Ovl_Mix_Boost = 1.5;
vec2 Ovl_Dir = vec2(0.12, .15);
...
/*
// Simple Mix Process
vec3 overlay(vec3 base, vec3 blend, float v_mix){
    return mix(2.0 * base * blend, 1.0 - 2.0 * (1.0 - base) * (1.0 - blend), v_mix);
}
*/

// Fancier Mix Process
vec3 overlay(vec3 base, vec3 blend, float v_mix){
    vec3 limit = step(v_mix, base); // step(0.5, base)
...
P_COLOR vec4 FragmentKernel( P_UV vec2 UV )
{
    ...
    //=== Apply Overlay
    vec4 blend = texture2D( OVERLAY, UV + ( Ovl_Dir * Ovl_Dir * TIME)) * Ovl_Col_Rate * nb_is_tween_col;
    vec4 c_ovl = blend * when_eq( NB_UsingOverlay, 1 ) ;

    COLOR.rgb = overlay( COLOR.rgb, c_ovl.rgb, .5 ) ;
    COLOR *= Ovl_Mix_Boost;
