...
uniform float scan_line_amount = 1.0; // :hint_range(0.0, 1.0) 
uniform float warp_amount = 0.1; // :hint_range(0.0, 5.0) 
uniform float noise_amount = 0.03; // :hint_range(0.0, 0.3)
uniform float interference_amount = 0.2; // :hint_range(0.0, 1.0)
uniform float grille_amount = 0.1; // :hint_range(0.0, 1.0)
uniform float grille_size = 1.0; // :hint_range(1.0, 5.0)
uniform float vignette_amount = 0.6; // :hint_range(0.0, 2.0)
uniform float vignette_intensity = 0.4; // : hint_range(0.0, 1.0)
uniform float aberation_amount = 0.5; // :hint_range(0.0, 1.0)
uniform float roll_line_amount = 0.3; // :hint_range(0.0, 1.0)
uniform float roll_speed = 1.0; // :hint_range(-8.0, 8.0)
uniform float scan_line_strength = -8.0; // :hint_range(-12.0, -1.0)
uniform float pixel_strength = -2.0; // :hint_range(-4.0, 0.0)
...
vec3 fetch_pixel(vec2 uv, vec2 off){
  ...
  vec3 clr = texture2D(CoronaSampler0 , pos, -16.0).rgb + noise;
  return clr;
}
